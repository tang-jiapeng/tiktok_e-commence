...
func GetUserBasicInfoById(db *gorm.DB, ctx context.Context, userId int32) (user *UserBasicInfo, err error) {
	err = db.WithContext(ctx).Model(&User{}).
		Select("username", "email", "sex", "description", "avatar").
		Where(&User{Base: Base{ID: userId}}).First(&user).Error
	return
}
...
func UpdateUser(db *gorm.DB, ctx context.Context, id int32, username string, email string, sex int32, description string, avatar string) error {
	...
	return db.WithContext(ctx).Model(&user).Select("username", "email", "sex", "description", "avatar").
		Updates(
			&User{
				Username:    username,
				Email:       email,
...
func GetUserRoleById(db *gorm.DB, ctx context.Context, id int32) (role string, err error) {
	var user *User
